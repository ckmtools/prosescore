...
var textlens = (() => {
  ...
  function isInitialPattern(text, dotIndex) {
  ...
  function fleschReadingEase(stats) {
    ...
    const score = 206.835 - 1.015 * (stats.words / stats.sentences) - 84.6 * (stats.syllables / stats.words);
    let grade;
    let interpretation;
    if (score >= 90) {
      grade = 5;
  ...
  // src/readability/flesch-kincaid.ts
  function fleschKincaidGrade(stats) {
    if (stats.words === 0 || stats.sentences === 0) {
      return { formula: "Flesch-Kincaid Grade Level", score: 0, grade: 0, interpretation: "N/A" };
    }
    const raw = 0.39 * (stats.words / stats.sentences) + 11.8 * (stats.syllables / stats.words) - 15.59;
  ...
  // src/readability/coleman-liau.ts
  function colemanLiauIndex(stats) {
    if (stats.words === 0 || stats.sentences === 0) {
    ...
    if (grade <= 0) {
      ...
      interpretation = `${grade}${ordinalSuffix2(grade)} grade`;
    } else {
      interpretation = "College level";
    }
  ...
  function automatedReadabilityIndex(stats) {
    ...
    if (grade <= 0) {
      ...
      interpretation = `${grade}${ordinalSuffix3(grade)} grade`;
    } else {
      interpretation = "College level";
    }
    return { formula: "Automated Readability Index", score, grade, interpretation };
  }
  function ordinalSuffix3(n) {
    if (n >= 11 && n <= 13) return "th";
    switch (n % 10) {
      case 1:
        return "st";
  ...
  // src/readability/gunning-fog.ts
  function gunningFogIndex(text) {
    ...
    if (grade <= 0) {
      ...
      interpretation = `${grade}${ordinalSuffix4(grade)} grade`;
    } else {
      interpretation = "College level";
    }
    return { formula: "Gunning Fog Index", score, grade, interpretation };
  }
  function ordinalSuffix4(n) {
    if (n >= 11 && n <= 13) return "th";
    switch (n % 10) {
      case 1:
        return "st";
  ...
  function smogIndex(text) {
    ...
    if (grade <= 0) {
      ...
      interpretation = `${grade}${ordinalSuffix5(grade)} grade`;
    } else {
      interpretation = "College level";
    }
    return { formula: "SMOG Index", score, grade, interpretation };
  }
  function ordinalSuffix5(n) {
    if (n >= 11 && n <= 13) return "th";
    switch (n % 10) {
      case 1:
        return "st";
  ...
  // src/data/dale-chall-words.ts
  var DALE_CHALL_WORDS = /* @__PURE__ */ new Set([
    "a",
    "able",
    "aboard",
    "about",
  ...
  // src/readability/linsear-write.ts
  function linsearWriteFormula(text) {
    ...
    if (gradeRounded <= 0) {
      ...
      interpretation = `${gradeRounded}${ordinalSuffix6(gradeRounded)} grade`;
    } else {
      interpretation = "College level";
    }
    return { formula: "Linsear Write Formula", score: grade, grade: gradeRounded, interpretation };
  }
  function ordinalSuffix6(n) {
    if (n >= 11 && n <= 13) return "th";
    switch (n % 10) {
      case 1:
        return "st";
  ...
  // src/analysis/keywords.ts
  function keywords(text, options) {
    ...
    for (const [word, count] of freq) {
      ...
      const density2 = count / totalWords * 100;
      results.push({ word, score, count, density: density2 });
    }
    results.sort((a, b) => b.score - a.score);
    return results.slice(0, topN);
  }

  // src/analysis/density.ts
  function density(text) {
    ...
    const totalWords = allWords.length;
    const unigramFreq = /* @__PURE__ */ new Map();
    ...
    const trigramFreq = /* @__PURE__ */ new Map();
  ...
  function sentiment(text) {
    ...
    let rawScore = 0;
    const positiveSet = /* @__PURE__ */ new Set();
    ...
    const label = comparative > 0.05 ? "positive" : comparative < -0.05 ? "negative" : "neutral";
  ...
  // src/analysis/seo.ts
  function seoScore(text, options) {
  ...
  function summarize(text, options) {
    ...
    if (sentences.length <= numSentences) {
      const summaryWords2 = sentences.reduce((sum, s) => sum + tokenizeWords(s).length, 0);
      return {
        sentences: [...sentences],
        ratio: totalWords > 0 ? summaryWords2 / totalWords : 0
      };
    }
    const allWords = tokenizeWords(text).map((w) => w.toLowerCase());
    ...
    for (let i = 0; i < total; i++) {
      ...
      if (wc >= 15 && wc <= 25) {
        ...
        lengthScore = Math.max(0.2, 1 - (wc - 25) / 50);
      }
